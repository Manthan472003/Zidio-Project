...
const sendTaskFromQAToDashboard = async (req, res) => {
    const { id } = req.params;
    if (!id) {
        return res.status(400).json({ message: 'Task ID parameter is required.' });
    }
    try {
        ...
        task.sentToQA = false;
        await task.save();
        return res.status(200).json({ message: 'Task sent To Dashboard successfully.' });
    } catch (error) {
        console.error('Error sending task to Dashboard :', error);
        return res.status(500).json({ message: 'Error sending task to Dashboard.' });
    }
};
