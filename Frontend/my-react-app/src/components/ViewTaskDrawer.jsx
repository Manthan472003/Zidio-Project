...
import { updateTask, sendFromQAToDashboard } from '../Services/TaskService';
...
import AddMediaTextInViewTask from './AddMediaTextInViewTask';
import ViewMediaShow from './ViewMediaShow';

const ViewTaskDrawer = ({ isOpen, onClose, task, tags, tasks, onUpdate = () => { }, onStatusChange }) => {
  ...
  const [previousAssignedUser, setPreviousAssignedUser] = useState(task?.taskAssignedToID || null);
  const [assignedUserEmail, setAssignedUserEmail] = useState('');
  const [previousDueDate, setPreviousDueDate] = useState(task?.dueDate || '');
  ...
  const { isOpen: isAddMediaOpen, onClose: onAddMediaClose } = useDisclosure();
  const [taskToComplete, setTaskToComplete] = useState(null);
  const isSentToQA = localTask.sentToQA === true;
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const commentData = {
        textCommentforViewtask: newComment,
        taskId: task.id,
  ...
  return (
    <>
      <Drawer onClose={onClose} isOpen={isOpen} size={size} closeOnOverlayClick={false}>
                      ...
                      return (
                        ...
                        >
                          <Box flex="1" overflow="hidden" textOverflow="ellipsis" whiteSpace="normal" width={12}>
                            ...
                            {comment.commentText && comment.commentText.length > 0 ? (
                              <>
                                <ViewMediaShow
                                  comment={comment}
                                />
                              ...
                              <Text>{comment.textCommentforViewtask}</Text>
                            )}
                    ...
                    <InputGroup size="lg">
                      ...
                      <InputRightElement>
                        <IconButton
                          ...
                          variant={"ghost"}
                          onClick={handleAddMediaToComment}
                        />
                      </InputRightElement>
                    </InputGroup>
        ...
        <AddMediaTextInViewTask
          isOpen={isModalOpen}
          onClose={handleCloseModal}
          taskId={task.id}
          onUploadComplete={fetchComments}
        />
