...
export const sendFromQAToDashboard = async (taskId) => {
    try {
        const response = await axios.put(`${API_URL}/sendFromQAToDashboard/${taskId}`);
        return response.data; // Return the success message
    } catch (error) {
        console.error('Failed to send task to Dashboard:', error.response ? error.response.data : error.message);
        throw error; // Rethrow error to be handled by the caller
    }
};